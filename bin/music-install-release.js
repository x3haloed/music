...
try {
  ...
  const installations = resolve(installationsArgument ?? join(homedir(), '.local', 'share', 'music-v2', 'installations'));
  if (git(source, ['status', '--porcelain', '--untracked-files=all']).length > 0) throw new Error('source repository must be clean before installing a release');
