...
test('attention progressively indexes low-salience material under its target without destroying its exact reference', () => {
  const exactSubject = { format: 'music-v4-object-1', sha256: 'a'.repeat(64), bytes: 123, mediaType: 'application/json' };
  const projection = {
    ...
    causalTrail: Array.from({ length: 12 }, (_, succession) => ({ succession, subjectId: `subject-${succession}`, evidence: exactSubject, detail: 'structural-card' })),
  };
  ...
  assert.ok(result.attentionManifest.indexedCausalCards > 0);
...
test('provider input overflow is a permanent local failure before network inference', async () => {
  ...
  await assert.rejects(() => actor.invoke({ role: 'select', schema: RoleSchemas.select, task: 'Select.', projection: { subject: { memory: 'x'.repeat(70_000) } } }), error => {
