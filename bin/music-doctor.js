...
try {
  ...
  const paths = git(root, ['ls-files', 'src/*.js', 'bin/*.js', 'package.json', 'package-lock.json'])
    .trim().split('\n').filter(Boolean).sort();
  const files = paths.map(path => compare(root, path));
  const changed = files.filter(file => !file.matches);
  if (command === 'check') {
    process.stdout.write(`${JSON.stringify({ ok: changed.length === 0, root, checked: files.length, changed: changed.map(publicDifference) }, null, 2)}\n`);
...
function compare(root, path) {
  ...
  return { path, matches: actual !== null && expected.equals(actual), expectedSha256: sha256(expected), actualSha256: actual === null ? null : sha256(actual), missing: actual === null, expected };
}
...
function publicDifference({ expected: _, ...difference }) { return difference; }
function git(root, args, encoding = 'utf8') { return execFileSync('git', ['-C', root, ...args], { encoding, maxBuffer: 8 * 1024 * 1024 }); }
function sha256(bytes) { return createHash('sha256').update(bytes).digest('hex'); }
function safeTimestamp() { return new Date().toISOString().replaceAll(':', '-'); }
function executable(path) { return path === 'src/cli.js' || path.startsWith('bin/'); }
function usage() { throw new Error('usage: music-doctor <check|restore> [REPOSITORY_ROOT]'); }
